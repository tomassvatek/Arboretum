...
namespace Arboretum.Common.Geolocation
{
    ...
    {   
        public static double Calculate(double originLat, double originLon, double targetLat, double targetLon)
        {   
            var r = 6372.8;
            ...
            var a = Math.Sin(dLat / 2) * Math.Sin(dLat / 2) + Math.Sin(dLon / 2) * Math.Sin(dLon / 2) * Math.Cos(latOrigin) * Math.Cos(latDestination);
